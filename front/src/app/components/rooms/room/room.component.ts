...
export class RoomComponent implements OnInit{
  ...
  constructor(private _userService: UserService, private _auctionService: AuctionService, private _flightService: FlightService, public router: ActivatedRoute) { 
  ...
  ngOnInit() {
    ...
    setInterval(() => {
      /* this.servis.bids = []; */
      this._auctionService.getBid(this._auctionService.rooms[this._auctionService.index].roomID).subscribe((res) => {
  ...
  onBidSubmit(){
    ...
    this.router.params.subscribe((parms) => {
      ...
      this._auctionService.setBid({flightID: parms['flightID'], roomID: this._auctionService.rooms[this._auctionService.index].roomID, email: this._userService.emailUser, bid: this.biding_Price}).subscribe((res) => {});
    })
